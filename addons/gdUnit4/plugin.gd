...
func _enter_tree() -> void:
	...
	var exclude_addons = ProjectSettings.get_setting("debug/gdscript/warnings/exclude_addons")
	...
	if Engine.get_version_info().hex < 0x40500:
		prints(
			"This GdUnit4 plugin version '%s' requires Godot version '4.5' or higher to run." % GdUnit4Version.current()
		)
		return
	...
	@warning_ignore("unsafe_method_access") await _gd_console.setup_update_notification(control)
	...
	@warning_ignore("return_value_discarded") resource_saved.connect(_on_resource_saved)
...
func _exit_tree() -> void:
	...
	@warning_ignore("unsafe_method_access") gdUnitTools.dispose_all(true)
...
func _add_context_menus() -> void:
	if Engine.get_version_info().hex >= 0x40400:
		...
		_gd_filesystem_context_menu = _preload_gdx_script(
