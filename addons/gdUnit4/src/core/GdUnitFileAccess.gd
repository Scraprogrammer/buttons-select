...
static func temp_dir() -> String:
	if not DirAccess.dir_exists_absolute(GDUNIT_TEMP):
		@warning_ignore("return_value_discarded") DirAccess.make_dir_recursive_absolute(GDUNIT_TEMP)
	return GDUNIT_TEMP
...
static func create_temp_dir(folder_name: String) -> String:
	...
	if not DirAccess.dir_exists_absolute(new_folder):
		@warning_ignore("return_value_discarded") DirAccess.make_dir_recursive_absolute(new_folder)
	return new_folder
	...
	if dir != null:
		...
		if error != OK:
			return GdUnitResult.error(
				"Can't copy file form '%s' to '%s'. Error: '%s'" % [from_file, to_file, error_string(error)]
			)
...
static func copy_directory(from_dir: String, to_dir: String, recursive: bool = false) -> bool:
	...
	if source_dir != null:
		@warning_ignore("return_value_discarded") source_dir.list_dir_begin()
		var next := "."
		...
		while next != "":
			...
			if source_dir.current_is_dir():
				if recursive:
					@warning_ignore("return_value_discarded") copy_directory(source + "/", dest, recursive)
				continue
...
static func delete_directory(path: String, only_content := false) -> void:
	...
	if dir != null:
		...
		@warning_ignore("return_value_discarded") dir.list_dir_begin()
		var file_name := "."
		while file_name != "":
...
static func scan_dir(path: String) -> PackedStringArray:
	...
	while next != "":
		...
		if next.is_empty() or next == "." or next == "..":
			continue
		@warning_ignore("return_value_discarded") content.append(next)
	return content
...
static func resource_as_array(resource_path: String) -> PackedStringArray:
	...
	while not file.eof_reached():
		@warning_ignore("return_value_discarded") file_content.append(file.get_line())
	return file_content
...
static func extract_zip(zip_package: String, dest_path: String) -> GdUnitResult:
	...
	if err != OK:
		return GdUnitResult.error(
			"Extracting `%s` failed! Please collect the error log and report this. Error Code: %s" % [zip_package, err]
		)
	...
	for zip_entry in zip_entries:
		...
		if zip_entry.ends_with("/"):
			@warning_ignore("return_value_discarded") DirAccess.make_dir_recursive_absolute(new_file_path)
			continue
		var file: FileAccess = FileAccess.open(new_file_path, FileAccess.WRITE)
		file.store_buffer(zip.read_file(zip_entry))
	@warning_ignore("return_value_discarded") zip.close()
